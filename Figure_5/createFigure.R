...
processRaw <- function(out, name)
{
    ...
    initProp <- rep(out$typeFreq[1], N)
    finalProp <- rep(out$typeAProp, N)
    return(cbind(time, nCells, nTypes, runId, repId, initProp, finalProp))
}
...
rawDF <- data.frame(time=rawMat[,1], nCells=rawMat[,2], nTypes=rawMat[,3],
    run_id=rawMat[,4], rep_id=rawMat[,5], initProp=rawMat[,6],
    finalProp=rawMat[,7])

rawDF <- subset(rawDF, time <= 48)
...
fig <- ggplot(subset(rawDF, nTypes==2)) +
    geom_point(aes(x=initProp, y=finalProp)) +
    labs(title="Outgrowth of Faster Growing Cell Type", 
        caption="Figure 5e, two cell types, A and B, with mean cycle of 12 and 36 hours",
        x="Initial Type A Proportion", y="Final Type A Proportion")
