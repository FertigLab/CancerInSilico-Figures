...
fig <- ggplot(subset(df, initialDensity==0.1), aes(x=time)) +
    geom_line(aes(y=nCells, linetype=boundary, color=factor(cycleLength),
        group=interaction(cycleLength, boundary))) +
    ...
    labs(title = "Sensitivity to Growth Rate and Boundary Presence", 
        ...
        color = "Mean Cell Cycle (hrs)") 
ggsave(filename='fig2a.pdf', plot=fig)

fig <- ggplot(subset(df, cycleLength==24), aes(x=time)) +
    geom_line(aes(y=density, linetype=boundary, color=factor(initialDensity),
        group=interaction(initialDensity, boundary))) +
    scale_linetype_manual(values=c("dashed", "solid")) +
